            ...
            <div class="h-96 border-2 rounded-md overflow-auto">
                <div class="w-full h-auto p-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 grid-rows-auto mb-4">
                    <div v-for="image in selectedImages" :key="image.id" class="w-full">
                        <img class="rounded-md h-36 w-full cursor-pointer" :src="image" alt="">
                    </div>
                </div>
            </div>
        <button @click="checkImages" class="btn-primary bg-black my-4">Next step</button>
...
<div class="absolute left-10 bottom-20">
        <NuxtLink to="/details" class="btn-primary bg-black">Back</NuxtLink>
    </div>
</div>
